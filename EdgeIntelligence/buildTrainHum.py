...
""" def visualize_loss(history, title):
    ...
    plt.ylabel("Loss")
    plt.legend()
    plt.yticks(np.arange(0, 5, 0.025))
    plt.show() 
STATISTICS"""
...
try:
    initialDataset= pd.read_csv(datasetPath, ",")
        ...
        print(e)
        print("Hum Training Error -> Cannot access humidity dataset")
"""loading dataset"""
...
try:
    ...
    print("Number of neurons for the second layer -> " + str(neuronsLayer2))
        ...
        print(traceback.format_exc())
        print(e)
        print("Hum Training Error -> Cannot access the configuration file")
...
try:
    ...
    model.add(BatchNormalization(name='BatchNorm_layer12'))
    ...
    model.compile(loss='mean_squared_logarithmic_error', optimizer=opt, metrics=['msle'])
        ...
        print(traceback.format_exc())
        print(e)
        print("Hum Training Error -> Cannot build the network")
"""Building"""


try:
    early_stopping = EarlyStopping(monitor='val_loss', patience=15)
    model_checkpoint = ModelCheckpoint('smartBuildingResources/checkpointHum/'+'model_hum_{epoch:02d}-{val_loss:.2f}.h5', monitor='val_loss', verbose=0, save_best_only=True, save_weights_only=False, mode='auto')
    ...
    #visualize_loss(history, "best configuration") for statistics
        ...
        print(traceback.format_exc())
        print(e)
        print("Hum Training Error -> Cannot train the network")
"""Training"""
