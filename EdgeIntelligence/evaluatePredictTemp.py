...
def getBestModel():
    file_path = 'smartBuildingResources/checkpointTemp/*.h5'
    try:
        ...
        print(e)
        print("Temp Prediction Error -> Cannot retrieve the best model for the network")
...
try:
    initialDataset= pd.read_csv(datasetPath, ",")
        ...
        print(traceback.format_exc())
        print(e)
        print("Temp Prediction Error -> Cannot retrieve the temperature dataset")
"""loading dataset"""
...
try:
    bestModel = tf.keras.models.load_model(getBestModel())
        ...
        print(traceback.format_exc())
        print(e)
        print("Temp Prediction Error -> Cannot load the best model")
...
try:
    ...
    print("test loss, test accuracy:", evaluationResults)
        ...
        print(traceback.format_exc())
        print(e)
        print("Temp Prediction Error -> Cannot perform the evaluation of the network")
"""Evaluation"""
...
try:
    ...
    print("Temperature Predicted -> " + formattedResult )
except Exception as e:
        print(traceback.format_exc())
        print(e)
"""Prediction"""
...
try:
    ...
    json_object["predictedTemperature"] = float(formattedResult)
    with open('smartBuildingResources/results.json', 'w') as json_output:
        json.dump(json_object, json_output, indent=4)
except Exception as e:
        print(traceback.format_exc())
        print(e)
        print("Temp Prediction Error -> Cannot access the results file")
...
tflite_model = converter.convert()
 ...
 #Save the model.
with open('checkpointTemp/model.tflite', 'wb') as f:
